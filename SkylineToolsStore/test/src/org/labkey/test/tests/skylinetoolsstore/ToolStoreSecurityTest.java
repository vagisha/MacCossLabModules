...
public class ToolStoreSecurityTest extends BaseWebDriverTest implements PostgresOnlyTest
{
    ...
    /**
     * ValidEmail rejects anything that is not an address, and the exception used to escape both
     * callers as a 500 and a mothership report. A bare word belongs in the "unknown users" message
     * the very next lines already render.
     */
    @Test
    public void testAMalformedOwnerAddressIsReportedNotAServerError()
    {
        // Not an address under any configuration. A bare word will not do - with a default domain
        // configured, ValidEmail completes it and it parses.
        String reply = postOwnersOnly("not a valid address@@example.test");

        assertFalse("A malformed owner address produced a server fault. The reply carried a stack " +
                        "trace instead of the refusal the next lines of the action already render",
                reply.contains("Exception") || reply.contains("at org.labkey"));
        assertTrue("The refusal should name the entry it could not use, got: " +
                        StringUtils.abbreviate(reply, 500),
                reply.contains(UNKNOWN_USERS_MESSAGE));
    }
